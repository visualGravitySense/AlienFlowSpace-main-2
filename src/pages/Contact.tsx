...
import { Send, Mail, MessageSquare, ChevronRight, Terminal, Shield, Scale, BookOpen, Users, Loader2, Radio, Copy, Map } from 'lucide-react';
...
const Contact: React.FC = () => {
  ...
  const SUPPORT_EMAIL = 'alien69flow@proton.me';
  ...
  const copyEmail = async (e: React.MouseEvent) => {
    ...
    try {
      ...
      setEmailCopied(true);
      toast.success('Email copiado', { description: SUPPORT_EMAIL });
  ...
  const notifyMeComingSoon = (platform: string) => {
    toast.success('¡Te avisaremos!', { description: `Te notificaremos cuando ${platform} esté disponible.` });
  };
  ...
  useEffect(() => {
    const handleMouseMove = (e: MouseEvent) => {
      ...
      formRef.current.style.transform = `translate(${moveX}px, ${moveY}px)`;
    };
  ...
  const handleFormSubmit = async (e: React.FormEvent) => {
    ...
    if (!result.success) {
      ...
      result.error.errors.forEach(err => {
        if (err.path[0]) errors[err.path[0] as keyof ContactFormData] = err.message;
      });
    ...
    try {
      const mailtoLink = `mailto:${SUPPORT_EMAIL}?subject=${encodeURIComponent(formData.subject)}&body=${encodeURIComponent("De: " + formData.name + "\nEmail: " + formData.email + "\n\nMensaje:\n" + formData.message)}`;
      setShowSuccessOverlay(true);
      setFormData({ name: '', email: '', subject: '', message: '' });
      setTimeout(() => window.location.href = mailtoLink, 500);
      setTimeout(() => setShowSuccessOverlay(false), 4000);
    } catch {
      toast.error('Error al enviar', { description: 'Por favor intenta de nuevo o contacta por email directamente.' });
    } finally {
  ...
  const socialPrimary = [
    { icon: 'telegram', name: 'Telegram', link: 'https://t.me/AlienFlow', text: '@AlienFlow', microcopy: 'Únete a la tripulación en Telegram', live: true },
    { icon: 'x', name: 'X (Twitter)', link: 'https://x.com/alien69flow', text: '@alien69flow', microcopy: 'Síguenos en X' },
    { icon: 'github', name: 'GitHub', link: 'https://github.com/Alien69Flow', text: 'Alien69Flow', microcopy: 'Código abierto' },
    { icon: 'gitbook', name: 'GitBook', link: 'https://alienflowspace.gitbook.io/DAO', text: 'Documentation', microcopy: 'Docs y roadmap', isNew: true },
  ];
  const socialSecondary = [
  ...
  const socialComingSoon = [
    { icon: 'discord', name: 'Discord', text: 'discord.gg/alienflow' },
    { icon: 'tiktok', name: 'TikTok', text: '@alienflowspace' },
  ];
              ...
              <AnimatePresence>
                {showSuccessOverlay && (
                  <motion.div
                    ...
                    className="absolute inset-0 bg-gradient-to-br from-alien-green/20 to-alien-gold/10 backdrop-blur-sm z-50 flex items-center justify-center rounded-2xl"
                  >
                    <motion.div initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} className="text-center p-8">
                      <motion.div
                        animate={{ scale: [1, 1.2, 1], rotate: [0, 10, -10, 0] }}
                        transition={{ duration: 0.6, repeat: Infinity, repeatDelay: 2 }}
                        className="mb-4"
                      >
                        <Radio className="w-16 h-16 text-alien-green mx-auto drop-shadow-[0_0_20px_rgba(57,255,20,0.8)]" />
                      </motion.div>
                      <h3 className="text-3xl font-nasalization text-alien-green mb-2 drop-shadow-[0_0_10px_rgba(57,255,20,0.5)]">
                        TRANSMISSION CONFIRMED
                      </h3>
                      <p className="text-alien-gold font-exo text-lg">Signal received. Response incoming...</p>
                      <div className="mt-4 flex gap-2 justify-center">
                        {[0, 1, 2].map((i) => (
                          <motion.div
                            ...
                            className="w-2 h-2 bg-alien-green rounded-full"
                          />
                        ))}
                      </div>
                    </motion.div>
              ...
              <form onSubmit={handleFormSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {[
                  ...
                  <div key={id} className="space-y-2">
                    <Label htmlFor={id} className="text-gray-300 font-exo">{label}</Label>
                    <Input
                      id={id}
                      name={id}
                      type={type || 'text'}
                    ...
                    />
                    {formErrors[id as keyof ContactFormData] && (
                      <motion.p initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="text-red-400 text-sm flex items-center gap-1">
                        <span className="text-red-500">⚠</span> {formErrors[id as keyof ContactFormData]}
                      </motion.p>
                    )}
                  </div>
                ))}
                <div className="md:col-span-2 space-y-2">
                  ...
                  {formErrors.subject && (
                    <motion.p initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="text-red-400 text-sm flex items-center gap-1">
                      <span className="text-red-500">⚠</span> {formErrors.subject}
                    </motion.p>
                  )}
                </div>
                <div className="md:col-span-2 space-y-2">
                  ...
                  {formErrors.message && (
                    <motion.p initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="text-red-400 text-sm flex items-center gap-1">
                      <span className="text-red-500">⚠</span> {formErrors.message}
                    </motion.p>
                  )}
                </div>
                <div className="md:col-span-2 space-y-3">
                  <motion.div
                    ...
                    animate={{
                      filter: [
                        ...
                        'drop-shadow(0 0 25px rgba(57,255,20,0.8)) drop-shadow(0 0 40px rgba(57,255,20,0.4))',
                        'drop-shadow(0 0 10px rgba(57,255,20,0.5))',
                      ],
                    }}
                  ...
                  <p className="text-xs text-gray-400 font-exo text-center md:text-left">
                    <Shield className="w-3 h-3 inline mr-1 text-alien-green" />
                    Sus datos están encriptados en todos los cuadrantes.
                  </p>
                </div>
              </form>
            </div>
              ...
              <AnimatePresence>
                {terminalHistory.map((entry, idx) => (
                  <motion.div
                    ...
                    animate={{ opacity: 1, y: 0 }}
                    className={
                      entry.type === 'user' ? 'text-alien-gold' :
                      entry.type === 'system' ? 'text-gray-500' : 'text-alien-green'
                    }
                  >
